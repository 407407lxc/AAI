...
def start_aai(
    ...
    workflow_json = init_workflow(
        campaign_id,
        task.definition,
        task.config_path,
        objective="AAI Codex-backed optimization campaign",
        planner_backend="rule",
        executor_backend="codex",
        evaluator_backend="modal-full",
        parent_id="baseline",
    )
    ...
    runtime.write_environment_snapshot({"task": task, "codex_api_key_env": codex_api_key_env, "workflow_json": str(workflow_json)})
    ...
    runtime.finish(status, {"campaign_id": campaign_id, "definition": task.definition, "workflow_json": str(workflow_json)})
