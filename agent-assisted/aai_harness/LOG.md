...
- Removed legacy retained-kernel directories from `agent-assisted/`:
  - `moe_fp8_block_scale_ds_routing_topk8_ng8_kg4_e32_h7168_i2048/`;
  - `gdn_decode_qk4_v8_d128_k_last/`;
  - `gdn_prefill_qk4_v8_d128_k_last/`;
  - `dsa_sparse_attention_h16_ckv512_kpe64_topk2048_ps64/`;
  - `dsa_topk_indexer_fp8_h64_d128_topk2048_ps64/`.
- Removed legacy `agent-assisted/skills/` and `agent-assisted/report.pdf`.
- Removed the old `full-agent/` trace/writeup package.
- Rewrote the root `README.md` as an AAI-only repository overview.
- Rewrote `agent-assisted/README.md` as an AAI-only package guide.
- Rewrote `agent-assisted/aai_harness/README.md` examples to use `<definition>/config.toml` placeholders instead of deleted retained-kernel paths.

### Notes

- Kept `agent-assisted/scripts/` because the current AAI evaluator flow still calls the pack/local/Modal evaluator scripts.
- AAI now expects target packages to be supplied externally or generated by the AAI workflow rather than vendoring historical retained kernels in this branch.
